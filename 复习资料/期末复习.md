...
2、假设有一个按需调页存储器，页表放在寄存器中。处理一个页错误，当有空的帧可用或被置换的帧没有被修改过时要用 8ms，当被置换的帧被修改过时用 20ms。存储器存取时间为 100ns。假设被置换的页中有 70%被修改过，有效存取时间不超过 200ns 时，最大可以接受的缺页率为多少？

答案：列出计算有效存取时间公式，EAT=(1-p)*100ns+p*t<=200ns，其中 p 为缺页率，t 为缺页时的有效访问时间列出 t 的计算公式，t=8ms*（1-70%）+20ms*70% =16.4ms 代入 EAT 公式，求出最大 p 为 1/163，约为 0.613%
